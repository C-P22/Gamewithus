...
def get_light_matrix(wall_matrix, target_tile, target_light_range, ignore_walls, light_matrix = [[]]):
    ...
    if light_matrix[target_tile[0]][target_tile[1]] >= target_light_range or \
            (wall_matrix[target_tile[0]][target_tile[1]] == 1 and not ignore_walls):
    ...
    if target_tile[0] != 0:
        light_matrix = get_light_matrix(wall_matrix, (target_tile[0] - 1, target_tile[1]), target_light_range - 1,
                                        False, light_matrix)
    if target_tile[1] != 0:
        light_matrix = get_light_matrix(wall_matrix, (target_tile[0], target_tile[1] - 1), target_light_range - 1,
                                        False, light_matrix)
